type GameState = "start" | "playing" | "paused" | "lose";
type PowerUpType = "triple" | "shield" | "laser";
...
interface RectCollide {
    x: number;
    y: number;
    width: number;
}
interface ImageWithCollide extends HTMLImageElement {
    rectCollide?: RectCollide;
}


const mouseTarget = document.getElementById("mouseTarget") as HTMLElement;
const canvas = document.querySelector("canvas") as HTMLCanvasElement;
const drawingSurface = canvas.getContext("2d") as CanvasRenderingContext2D;
...
const meteorImages: ImageWithCollide[] = [new Image(), new Image(), new Image(), new Image()];
...
const trashImages: HTMLImageElement[] = [new Image(), new Image(), new Image()];
...
const explosionImages: HTMLImageElement[] = [
...
const powerUpImages: Record<PowerUpType, HTMLImageElement> = {
...
let powerupMessage: PowerupMessage | null = null;

let gameState: GameState = "start";
let score = 0;

const scoreDisplay = document.getElementById("points") as HTMLElement;
const lifeText = document.getElementById("current-life") as HTMLElement;
const effectDisplay = document.getElementById("current-effect") as HTMLElement;

const rocket: Rocket = { x: 200, y: 650, width: 80, height: 80, speedX: 0, speedY: 0, life: 3 };

let bullets: Bullet[] = [];
let aliens: Alien[] = [];
let meteor: Meteor[] = [];
let explosions: AnimateGif[] = [];
let powerups: PowerUpEntity[] = [];
...
let alienInterval: ReturnType<typeof setInterval>;
...
function startGame(): void { //ANCHOR - Start
    rocket.life = 3;
    score = 0;
    trashCollected = 0;
    powerUp = null;
...
canvas.addEventListener("click", function (e: MouseEvent) { //ANCHOR - Btn EventListener
...
function shootBullet(): void { //ANCHOR - shootBullet
    switch (powerUp) {
        case "triple":
...
function spawnMeteor(): void {    //ANCHOR - spawnMeteor
    if (gameState !== "playing") return;
    const randomX = Math.floor(Math.random() * (canvas.width - 50));
...
function spawnPowerUp(x: number, y: number): void {
    const types: PowerUpType[] = ["triple", "shield", "laser"];
...
function collectPowerUp(type: PowerUpType): void {
...
class AnimateGif {
    ...
    constructor(imgArray: HTMLImageElement[], px: number, py: number, width: number, height: number) {
        this.imgs = imgArray;
        this.index = 0;
        this.px = px;
        this.py = py;
    ...
    animate(ctx: CanvasRenderingContext2D): void {
        ...
        if (this.frameTick >= this.frameDelay) {
            this.frameTick = 0;
            this.index++;
            if (this.index >= this.imgs.length) this.finish = true;
        }
    }
}


function update(): void { //ANCHOR - Update
    ...
    if (moveUp && !moveDown) rocket.speedY = -spd;
    else if (moveDown && !moveUp) rocket.speedY = spd;
    else rocket.speedY = 0;

    if (moveLeft && !moveRight) rocket.speedX = -spd;
    else if (moveRight && !moveLeft) rocket.speedX = spd;
    ...
    for (let t = trashItems.length - 1; t >= 0; t--) {
        ...
        if (trashItems[t].y > canvas.height) trashItems.splice(t, 1);
    }
    ...
    for (let e = explosions.length - 1; e >= 0; e--) {
        if (explosions[e].finish) explosions.splice(e, 1);
    }
    ...
    for (let b = bullets.length - 1; b >= 0; b--) {
        for (let a = aliens.length - 1; a >= 0; a--) {
            if (bullets[b] && aliens[a] &&
                ...
                bullets.splice(b, 1);
                if (Math.random() < 0.7) spawnTrash(aliens[a].x + aliens[a].width / 2 - 17, aliens[a].y);
                aliens.splice(a, 1);
                score++;
                updateUI();
                break;
    ...
    for (let a = aliens.length - 1; a >= 0; a--) { //ANCHOR - Alien
        if (rocket.x < aliens[a].x + aliens[a].width &&
            ...
            if (powerUp !== "shield") {
                rocket.life--;
                updateUI();
                if (rocket.life <= 0) { gameState = "lose"; return; }
            }
        }
    }
...
function render(): void { //ANCHOR - Render
...
type UpgradeType = keyof Upgrades;

function buyUpgrade(type: UpgradeType): void {
    const upg = upgrades[type];
    if (trashCollected < upg.cost) return;
...
function updateUpgradeButtons(): void {
    (document.getElementById("btn-speed") as HTMLButtonElement).innerHTML =
        `<i class="bi bi-speedometer2"></i> Speed (Niv.${upgrades.speed.level}) — ${upgrades.speed.cost} <i class="bi bi-trash3-fill"></i>`;
    (document.getElementById("btn-saver") as HTMLButtonElement).innerHTML =
        `<i class="bi bi-shield-fill"></i> Saver (Niv.${upgrades.saver.level}) — ${upgrades.saver.cost} <i class="bi bi-trash3-fill"></i>`;
    (document.getElementById("btn-damage") as HTMLButtonElement).innerHTML =
        `<i class="bi bi-crosshair2"></i> Damage (Niv.${upgrades.damage.level}) — ${upgrades.damage.cost} <i class="bi bi-trash3-fill"></i>`;
    (document.getElementById("btn-extralife") as HTMLButtonElement).innerHTML =
        `<i class="bi bi-heart-fill"></i> Extra Life (Niv.${upgrades.extraLife.level}) — ${upgrades.extraLife.cost} <i class="bi bi-trash3-fill"></i>`;

    (document.getElementById("btn-speed") as HTMLButtonElement).disabled = trashCollected < upgrades.speed.cost;
    (document.getElementById("btn-saver") as HTMLButtonElement).disabled = trashCollected < upgrades.saver.cost;
    (document.getElementById("btn-damage") as HTMLButtonElement).disabled = trashCollected < upgrades.damage.cost;
    (document.getElementById("btn-extralife") as HTMLButtonElement).disabled = trashCollected < upgrades.extraLife.cost;
}


function renderStartScreen(): void {  //ANCHOR - StartScreen
    drawingSurface.clearRect(0, 0, canvas.width, canvas.height);
    drawingSurface.textAlign = "center";
...
function renderLoseScreen(): void {   //ANCHOR - LoseScreen
    drawingSurface.clearRect(0, 0, canvas.width, canvas.height);
    drawingSurface.textAlign = "center";
...
function renderPauseScreen(): void {  //ANCHOR - PauseScreen
    render();
